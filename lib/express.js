...
var makeRoute = require('./route.js');
/**
...
function createApplication(){
    ...
    function innerNext(err){
      if(err){
        if(_this.stack.length > 0){
          ...
          req.url = next.originalUrl?next.originalUrl:req.url;
          next(err);
        }else{
          res.writeHead(500);
  ...
  methods.forEach(function(method){
    app[method] = function(path, handler){
      app.use(path, makeRoute(method, handler));
    };
  });
  return app;
};